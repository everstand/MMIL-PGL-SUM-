...
if __name__ == "__main__":
    ...
    for split_id in range(5):
        # Model data
        model_path = f"../PGL-SUM/experiments/pretrained_models/table{table}_models/{dataset}/split{split_id}"
        model_file = [f for f in listdir(model_path) if isfile(join(model_path, f))]

        # Read current split
        split_file = f"../PGL-SUM/data/datasets/splits/{dataset.lower()}_splits.json"
